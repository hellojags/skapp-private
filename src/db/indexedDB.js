...
export const IDB_STORE_REVISIONS = "revisions"
...
const revisionDataStoreIDB = localforage.createInstance({
  name: IDB_NAME,
  storeName: IDB_STORE_REVISIONS,
})

export const setJSONinDB = async (key, value, opts) => {
  const result = null
  try {
    if (opts.store === IDB_STORE_REVISIONS) {
      await revisionDataStoreIDB.setItem(key, value)
    } else {
...
export const setAllinDB = async (arrayOfJson, opts) => {
  const result = null
  try {
...
export const getJSONfromDB = async (key, opts) => {
  let value = null
  try {
    if (opts.store === IDB_STORE_REVISIONS) {
      value = await revisionDataStoreIDB.getItem(key)
    } else {
...
export const removeJSONfromDB = async (key, opts) => {
  const value = null
  try {
    if (opts.store === IDB_STORE_REVISIONS) {
      await revisionDataStoreIDB.removeItem(key)
    } else {
...
export const clearAllfromDB = async (opts) => {
  if (opts.store === IDB_STORE_REVISIONS) {
    await revisionDataStoreIDB
      .dropInstance({
        name: IDB_NAME,
