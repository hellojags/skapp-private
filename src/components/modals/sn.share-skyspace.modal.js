...
import {
  bsSaveSharedWithObj,
  bsSetSharedSkylinkIdxV2,
  bsShareSkyspaceV2,
  getSharedMasterJSONFromSkyDB,
  getSkySpace,
...
export default function SnShareSkyspaceModal(props) {
  ...
  const handleDeletedRecipients = async () => {
    ...
    for (const id of deletedIdList) {
      ...
      const idxCurrentSpace = props.sharedWithObj[id].spaces.indexOf(
        props.skyspaceName
      )
      ...
      if (props.sharedWithObj[id].spaces.length === 0) {
        ...
        for (const skyspaceName of props.sharedWithObj[id].spaces) {
          const skyspaceObj = await getSkySpace(stUserSession, skyspaceName)
      ...
      if (skylinkListById[id]) {
        ...
        props.sharedWithObj[id].skylinks = props.sharedWithObj[id].skylinks.filter(
          (skhubId) => skylinkList.indexOf(skhubId) > -1
        )
        const masterSharedFileJSON = await getSharedMasterJSONFromSkyDB(id)
        // Step1: updating List of Shared Skylinks
        const keyValueObj = await bsSetSharedSkylinkIdxV2(
          stUserSession,
          id,
          props.sharedWithObj[id].skylinks,
          props.sharedWithObj
        )
        ...
        await putFileForShared(
          stUserSession,
          SHARED_PATH_PREFIX + id,
          masterSharedFileJSON
        )
        ...
        await bsShareSkyspaceV2(
          props.userSession,
          props.sharedWithObj[id].spaces,
          props.sharedWithObj[id].userid,
          props.sharedWithObj
        )
      }
    }
      ...
      >
        ...
        <DialogContent>
          ...
          <Grid container spacing={1} direction="row">
            <Grid item xs={12}>
              <>
                {props.sharedWithObj &&
                  Object.keys(props.sharedWithObj)
                    .filter((key) => deletedIdList.indexOf(key) === -1)
                    .filter(
                      (id) =>
                        props.sharedWithObj[id].spaces.indexOf(props.skyspaceName) >
                        -1
                    )
                    .map((key, idx) => (
                      <Tooltip title={props.sharedWithObj[key].userid} arrow>
                        <Chip
                          key={idx}
                          label={cliTruncate(props.sharedWithObj[key].userid, 30)}
        ...
        <DialogActions>
          ...
          <Button
            ...
            autoFocus
            disabled={
              (recipientId == null || recipientId.trim() === "") &&
